...
def main():
    ...
    while t < T_final:
        ...
        iter += 1
        u = np.concatenate((u_A[:, :], u_B[1:, :]), axis=0)
        snapshot_list.append(u.copy())
        grid_x = np.concatenate((A_adrsolver.grid_x[:, :], B_adrsolver.grid_x[1:, :]), axis=0)
        grid_y = np.concatenate((A_adrsolver.grid_y[:, :], B_adrsolver.grid_y[1:, :]), axis=0)
    
    snapshots = np.stack(snapshot_list, axis=0)
    print("snapshots shape:", snapshots.shape)
    # 保存
    np.savez(
        f"{outdata_dir}/coupled_snapshots.npz",
        snapshots=snapshots,
    )
    print("Saved to solution_snapshots.npz")
    ...
    ax.set_ylabel('y')
    # plt.savefig(fig_dir / f'FULL_u_{iter}.pdf', dpi=300, bbox_inches='tight')
    plt.savefig(fig_dir / f'FULL_u.pdf', dpi=300, bbox_inches='tight')
    plt.close()
